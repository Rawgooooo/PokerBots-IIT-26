...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState) -> ActionFold | ActionCall | ActionCheck | ActionRaise | ActionBid:
        ...
        # ----------------------------------------------------------------------
        # 1. AUCTION PHASE (Fallback/Temporary Logic)
        # ----------------------------------------------------------------------
        if current_state.street == 'auction':
            # Bid a slightly random, small percentage of chips for now
            v = random.uniform(0, 0.3)
            return ActionBid(min(10, int(v * current_state.my_chips)))
        ...
        if current_state.street == 'pre-flop':
            ...
            if hand_equity >= (pot_odds + MARGIN):
                ...
                if hand_equity >= RAISE_THRESHOLD and current_state.can_act(ActionRaise):
                    # Raise the minimum for now as a baseline strong play
                    # In future iterations, we can size this better based on pot size
                    min_raise, max_raise = current_state.raise_bounds
                    # Raise smaller so we aren't completely risking it all
