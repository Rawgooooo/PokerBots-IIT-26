...
class PokerMatch():
    ...
    def __init__(self, bot1_name, bot1_file, bot2_name, bot2_file, small_log=False):
        self.small_log = small_log
        self.timestamp = datetime.now()
        self.bot1_name = bot1_name
        self.bot1_file = bot1_file
        self.bot2_name = bot2_name
        self.bot2_file = bot2_file
        self.log = [self.timestamp.strftime('%Y-%m-%d %H:%M:%S ') + bot1_name + ' vs ' + bot2_name]
    ...
    def run(self):
        ...
        players = [
            BotProcess(self.bot1_name, self.bot1_file),
            BotProcess(self.bot2_name, self.bot2_file)
        ]
        ...
        name = f"{all_bots[0].name}x{all_bots[1].name}-{self.timestamp.strftime('%m%d-%H%M-%f')}.glog"
...
if __name__ == '__main__':
    ...
    bots = [
        (BOT_1_NAME, BOT_1_FILE),
        (BOT_2_NAME, BOT_2_FILE),
        (BOT_3_NAME, BOT_3_FILE),
        (BOT_4_NAME, BOT_4_FILE)
    ]
    
    matchups = list(itertools.combinations(bots, 2))
    
    bot_stats = {b[0]: {'total_bankroll': 0, 'matches_played': 0} for b in bots}
    
    tour_time = datetime.now()
    summary_filename = tour_time.strftime("Tour_%m%d%H%M.txt")
    summary_path = os.path.join(GAME_LOG_FOLDER, summary_filename)
    os.makedirs(GAME_LOG_FOLDER, exist_ok=True)
    
    match_logs = []
    
    for round_iter in range(1, 4): # Play 3 times
        for b1, b2 in matchups:
            print("\n" + "="*50)
            print(f"--- TOURNAMENT MATCH: {b1[0]} vs {b2[0]} (Iteration {round_iter}/3) ---")
            print("="*50)
            match = PokerMatch(b1[0], b1[1], b2[0], b2[1], small_log=args.small_log)
            results = match.run()
            
            for bot in results:
                bot_stats[bot.name]['total_bankroll'] += bot.bankroll
                bot_stats[bot.name]['matches_played'] += 1
                
            match_logs.append(f"Iter {round_iter} - {results[0].name} ({results[0].bankroll}) vs {results[1].name} ({results[1].bankroll})")

    # Write summary
    with open(summary_path, 'w') as f:
        f.write(f"Tournament Summary - {tour_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("="*50 + "\n\n")
        ...
        for rank, (name, stats) in enumerate(sorted_bots, 1):
            f.write(f"{rank}. {name} | Total Bankroll: {stats['total_bankroll']} | Matches Played: {stats['matches_played']}\n")
            
    print(f"\nTournament complete. Overall summary written to {summary_path}")